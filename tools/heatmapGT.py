...
def gaussian2D_Rot(shape, sigma_x=1,sigma_y=1,theta=0):
    ...
    for i in range(len(x[0])):
        ...
        for j in range(len(y)):
            y_tmp = y[j][0]              
            #print(y_tmp)            
            res = - ( np.matmul(np.matmul(np.matrix([x_tmp ,y_tmp ]),np.linalg.pinv(rot_cov)),np.matrix([x_tmp ,y_tmp]).T))
...
def draw_umich_gaussian_rot(heatmap, center,sigma_x,sigma_y, theta= 0.5*np.pi, k=1):
    ...
    #print(gaussian)
    gaussian = gaussian2D_Rot((diameter, diameter), (2*sigma_x+1) / 6,(2*sigma_y+1)/6,theta)
...
def draw_msra_gaussian_rot(heatmap, center, sigma_x,sigma_y,theta=1):
    ...
    #tmp = np.array([[0.00001,0],[0,0.0001]])
    #print("Inverse pinv",np.linalg.pinv(rot_cov))
    #print("Inverse",np.matrix(rot_cov+tmp).I)
    #print("cov",cov)
...
class ctDataset(data.Dataset):
    ...
    def __init__(self, data_dir='/home/baode/data2/lijl/cheny_sapce/Dataset/obj/hsrc2016/', split='train'):#./data
        self.data_dir = os.path.join(data_dir, 'hsrc')#crack50
    ...
    def get_heatmapGT(self,save_path):
        for i in range(self.num_samples):
            ...
            #if img_path != "/home/baode/data2/lijl/cheny_sapce/R-CenterNet/data/crack50/images/7 (3)_1.jpg":
            #    continue
            ...
            if keep_res:
                ...
                input_h, input_w = 768,768 #512, 512 

            trans_input = get_affine_transform(c, s, 0, [input_w, input_h])
            inp_img = cv2.warpAffine(img, trans_input,(input_w, input_h),flags=cv2.INTER_LINEAR) 
            inp = (inp_img.astype(np.float32) / 255.)  
            ...
            for k in range(num_objs):   
                ...
                if h > 0 and w > 0:
                    if 1:
                        ...
                        draw_umich_gaussian_rot(hm[cls_id], ct_int, radius_x,radius_y,an*math.pi/180)                    
                        wh[k] = 1. * w, 1. * h
                        ang[k] = 1. * an
            ...
            hm_save = np.transpose(hm, (1,2,0))
            print(hm_save.shape)
            hm_save = np.squeeze(hm_save,2)
            print(hm_save.shape)
            print("file_name",file_name)
            tmp_save_path = save_path + '768HMmse'+file_name #+'.jpg' #+'.jpg' for crack set
            print("tmp_save_path",tmp_save_path)
            img_hm = cv2.resize(inp_img, (192, 192), interpolation=cv2.INTER_LINEAR)
            show_cam_on_image(img_hm,hm_save,tmp_save_path)
            #cv2.imwrite(tmp_save_path, hm_save*255.0)
            #test
            npy_name = file_name.split(".", 1)[0]
            print(npy_name)
            npy_save_path = save_path + '768HMmse' + npy_name  +'.npy'
            np.save(npy_save_path,hm) 
            hm_load = np.load(npy_save_path)
            #hm_load = cv2.imread(tmp_save_path,-1)
            #hm_load = np.expand_dims(hm_load,2)          
            print(hm_load.shape)
            #check
            arr_for_check = hm_load - hm
            if (arr_for_check == 0.0).all() :
                print("check success")
            else : 
                print("check fail")
                while(1):True
            
path_save =  "/home/baode/data2/lijl/cheny_sapce/Dataset/obj/hsrc2016/hsrc/768heatmap/"               
t_dataset = ctDataset(split='train')
t_dataset.get_heatmapGT(path_save)
v_dataset = ctDataset(split='val')
v_dataset.get_heatmapGT(path_save)
