...
def ennUpsample(inplanes,scale_factor=2,mode="bilinear",align_corners=False):
    in_type = FIELD_TYPE['regular'](gspace, inplanes)
    return enn.R2Upsampling(in_type, scale_factor=scale_factor, mode=mode, align_corners=align_corners)
...
class DeconvBlock(enn.EquivariantModule):
    ...
    def __init__(self,
        ...
        self.conv_cfg = conv_cfg
        self.norm_cfg = norm_cfg
        self.upsample = ennUpsample(in_channels)
        self.conv1 = build_conv_layer(
            conv_cfg,
            self.in_channels,
            self.out_channels,
