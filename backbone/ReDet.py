...
class ReResNet(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        ...
        if pretrained is None:
            ...
            #print(model_dict)
            modelCheckpoint = torch.load(pretrained)
...
class DeconvBlock(enn.EquivariantModule):
    """Deconv Bottleneck block 
    Args:
        in_channels (int): Input channels of this block.
        out_channels (int): Output channels of this block.
        expansion (int): The ratio of ``out_channels/mid_channels`` where
    ...
    def __init__(self,
        ...
        self.conv_cfg = conv_cfg
        self.norm_cfg = norm_cfg
        self.upsample = ennInterpolate(in_channels,scale_factor=2)
        self.conv1 = build_conv_layer(
            conv_cfg,
            self.in_channels,
            self.out_channels,
...
class deconv_Layer(nn.Sequential):
    def __init__(self,
                 in_channels,
                 out_channels,
                 kernel_size,
                 ...
                 **kwargs):
        layers = []
        print("deconv_Layer in_channels",in_channels)
        print("deconv_Layer out_channels",out_channels)
        layers.append(
            DeconvBlock(
                in_channels=in_channels,
                out_channels=out_channels,
        ...
        super(deconv_Layer, self).__init__(*layers)



class ReDeCovn(nn.Module):
    def __init__(self,inplanes,num_layers,num_filters,num_kernels, **kwargs):
        super(ReDeCovn, self).__init__()
        self.inplanes = inplanes
        self.deconv_num_layers = num_layers
        self.deconv_num_filters = num_filters
        self.deconv_num_kernels = num_kernels
        self.deconv_layers = [] 
        for i in range(self.deconv_num_layers):
            kernel, padding, output_padding = \
                self._get_deconv_cfg(self.deconv_num_kernels[i], i)            
            planes = self.deconv_num_filters[i]
            print("Number",i)
            print("self.inplanes",self.inplanes)
            print("planes",planes)
            de_res_layer = self.make_deconv_layer(
            ...
            layer_name = f'deconvlayer{i + 1}'
            self.add_module(layer_name, de_res_layer)
            self.deconv_layers.append(layer_name)  
    ...
    def make_deconv_layer(self, **kwargs):
        return deconv_Layer(**kwargs) 
    ...
    def forward(self,x):
        for i, layer_name in enumerate(self.deconv_layers):
            deres_layer = getattr(self, layer_name)
            x = deres_layer(x)
        return x

class fcHead(nn.Module):
    def __init__(self,heads, head_conv, plot, **kwargs):
        ...
        for head in sorted(self.heads):
            ...
            if head_conv > 0:
                fc = nn.Sequential(
                        nn.Conv2d(256, self.head_conv, kernel_size=3, padding=1, bias=True),
    ...
    def forward(self,x):
        ret = {}
        res = [] # for draw the result on the  picture 
        #radflyaer
        for head in self.heads:
            #print(head)
            #if head == 'wh'and utility_lite.RADFLayer =="True":
            #    tmp = self.radflayer(x)
            #else:
            #    # convert to tensor
            #tmp = x.tensor
            #print("tmp",tmp.shape)                    
            #print(" self.__getattr__(head)",self.__getattr__(head))
            ret[head] = self.__getattr__(head)(x)
            res.append(self.__getattr__(head)(x))  #ret
            #print("ret[head]",head,":",ret[head].shape) 
        return res if self.plot else ret      

class ReCenDet(nn.Module):
# used for deconv layers
    def __init__(self, depth,heads = utility_lite.heads, head_conv=256,pretrain='/mnt/cheny_worksapce/R-CenterNet/backbone/pth/re_resnet50_c8_batch256-25b16846.pth',plot=False, **kwargs):
        super(ReCenDet, self).__init__()
        self.depth = depth
        self.heads = heads
        self.head_conv = head_conv
        self.pretrain = pretrain #pretrain
        self.plot = plot
        self.deconv_num_layers = 3
        self.deconv_num_filters = [256, 256, 256]
        self.deconv_num_kernels = [4, 4, 4]
        assert self.deconv_num_layers == len(self.deconv_num_filters), \
            'ERROR: num_deconv_layers is different len(num_deconv_filters)'
        assert self.deconv_num_layers == len(self.deconv_num_kernels), \
            'ERROR: num_deconv_layers is different len(num_deconv_filters)' 
        if depth == 34:
            self.inplanes = 512 #64
        elif depth == 50 :
            self.inplanes = 512*4  
        if utility_lite.RADFLayer == "True":
            print("Initize with RADFLayer:")
            from RotAwarelayer import RADFLayer
            self.radflayer = RADFLayer(256,utility_lite.mid_feaC,256)
            self.radflayer.init_weights() 
        self.backbone = ReResNet(self.depth)
        self.backbone.init_weights(self.pretrain)
        self.ReDeCovn = ReDeCovn(self.inplanes,self.deconv_num_layers,self.deconv_num_filters,self.deconv_num_kernels)
        self.ReDeCovn.init_weights()
        self.fcHead = fcHead(self.heads, self.head_conv, self.plot)
        self.fcHead.init_weights()

    def forward(self,x):
        fea= self.backbone(x)
        #print(fea.shape)
        upfea = self.ReDeCovn(fea) 
        if utility_lite.RADFLayer =="True":
            upfea = self.radflayer(upfea)
        else:
            upfea = upfea.tensor    
        res = self.fcHead(upfea)
        return res
if 0:
    model = ReCenDet(depth=50,heads = {'hm': 1, 'wh': 2, 'ang':1, 'reg': 2}, head_conv=256,pretrain='/mnt/cheny_worksapce/R-CenterNet/backbone/pth/re_resnet50_c8_batch256-25b16846.pth',plot=False)
    model.eval()
    inputs = torch.rand(1, 3, 768, 768)
    level_outputs = model.forward(inputs)
    #print(level_outputs.shape)
    for level_out in level_outputs:
        print(level_outputs[level_out].shape)