# -*- coding: utf-8 -*-
"""
This file contains our implementation of ReResNet and rotation-aware dynamic filtering layer for centernet framework.
@author: Zhuangzzhuang chen
"""
...
import sys                    #导入sys模块
sys.path.append(r'/home/cheny/cheny_work_space/R-CenterNet')    #要用绝对路径
print(sys.path) 
import utility_lite
...
def deconvnxn(inplanes, out_planes, kernel_size=3, stride=1, padding=0,output_padding=0, groups=1, bias=False):
    print("inplanes",inplanes)
    print("out_planes",out_planes)
    in_type = FIELD_TYPE['regular'](gspace, inplanes)
    out_type = FIELD_TYPE['regular'](gspace, out_planes)    
    return enn.R2ConvTransposed(in_type, out_type, kernel_size,
                      stride=stride,
                      padding = padding,
...
def build_deconv_layer(cfg, *args, **kwargs):
    layer = deconvnxn(*args, **kwargs)
    return layer
...
class ReCenterResNet(nn.Module):#BaseBackbone
    """ReResNet backbone.
    ...
    def __init__(self,
        ...
        super(ReCenterResNet, self).__init__()
    ...
    def _make_deconv_layer(self, num_layers, num_filters, num_kernels):
        ...
        for i in range(num_layers):
            ...
            #def decovnxn(inplanes, outplanes, kernel_size=3, stride=1, padding=0,output_padding=0, groups=1, bias=False):
            layers.append(decovnxn(self.inplanes,
                                             planes,
                                             kernel_size=kernel,
                                             stride=2,
                                             padding=padding,
                                             output_padding=output_padding,
                                             bias=False))                    
            layers.append(build_norm_layer(self.norm_cfg,planes,4+i))
            layers.append(ennReLU(planes,inplace=True))
            self.inplanes = planes
        print(layers)
    ...
    def init_weights(self, pretrained=None):
        super(ReCenterResNet, self).init_weights(pretrained)
    ...
    def forward(self, x):
        ...
        for head in self.heads:
            ...
            ret[head] = self.__getattr__(head)(tmp)
            res.append(self.__getattr__(head)(tmp))  #ret
    ...
    def train(self, mode=True):
        super(ReCenterResNet, self).train(mode)
        self._freeze_stages()
...
def ReResNet(depth, heads = {'hm': 1, 'wh': 2, 'ang':1, 'reg': 2}, head_conv=256, plot=False):
    assert depth in [18,34,50,101,152], \
            'ERROR: depth must be in [18,34,50,101,152]'
    model = ReCenterResNet(depth, heads, head_conv=head_conv, plot=plot)
    return model
if 0:
    import torch
    model = ReResNet(18)
