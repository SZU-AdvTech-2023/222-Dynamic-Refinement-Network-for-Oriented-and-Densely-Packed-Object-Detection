...
class RegL1Loss(nn.Module):
  ...
  def forward(self, pred, mask, ind, target):
    ...
    loss = loss / (mask.sum() + 1e-4) # ÿ��Ŀ���ƽ����ʧ
    return loss
...
class CtdetLoss(torch.nn.Module):
    ...
    def __init__(self, loss_weight):
        ...
        #self.crit = FocalLoss()
        self.crit = torch.nn.MSELoss() if utility_lite.mse_loss else FocalLoss()
...
#        print(target_tensor['hm'].size())
        if utility_lite.mse_loss:
          hm_loss += self.crit(pred_tensor['hm'], target_tensor['hm'])*100
        else:
          hm_loss += self.crit(pred_tensor['hm'], target_tensor['hm'])
        if ang_weight > 0:
            pred_tensor['ang'] = _relu(pred_tensor['ang'])
            #print(pred_tensor['ang'].shape,target_tensor['reg_mask'].shape,target_tensor['ind'].shape,target_tensor['ang'].shape)
            #while(1):True
        ...
        print("hm: ",hm_loss.data, " wh: ",wh_loss.data, " ang: ",ang_loss.data, " offset: ",off_loss.data)     
